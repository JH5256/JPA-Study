...
import com.ohgiraffers.section03.entitiy.EntityLifeCycle;
import com.ohgiraffers.section03.entitiy.EntityManagerGenerator;
import com.ohgiraffers.section03.entitiy.Menu;
...
public class EntityLifeCycleTests {
    ...
    void testDetachEntity(int menuCode, int menuPrice) {
        ...
        // flush : 영속성 컨텍스트의 상태를 DB로 내보낸다. commit하지 않은 상태이므로 rollback이 가능하다.
        entityManager.flush();
    ...
    void testDetachAndMerge(int menuCode, int menuPrice) {
        ...
        // 조회한 영속 엔티티 객체에 준영속 상태의 엔티티 객체의 값을 병합 한 뒤 영속 엔티티 객체를 변환한다.
    ...
    void testClearPersistenceContext(int menuCode) {
        ...
        // when
        // clear : 영속성 컨텍스트를 초기화 한다. -> 영속성 컨텍스트 내의 모든 엔티티들을 준영속화 시킨다.
        entityManager.clear();
    ...
    void testRemoveEntity(int menuCode) {
        ...
        // 단, 트렌젝션을 제어하지 않으면 데이터베이스에서 영구 반영 되지는 않는다.
        // 트렌젝션을 커밋하는 순간 영속성 컨텍스트에서 관리하는 엔티티 객체가 데이터베이스에 반영된다.
        ...
        entityTransaction.rollback();   // 최종 커밋은 하지 않았기 때문에 rollback으로 db에는 연동이 안됨.
    }
